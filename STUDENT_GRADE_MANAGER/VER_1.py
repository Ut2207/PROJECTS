...
def interface():
    ...
    5)AVERGAE MARKS 
    6) QUIT""")
    check_choice=int(input("ENETR YOUR CHOICE:",))
    if check_choice==1:
        add_marks()
        return True
    elif check_choice==2:
        if check_choice==int(check_choice):
            delete_record=input("ENTER THE NAME OF THE UNFORTUNATE STUDENT:",)
        ...
        if check_choice==int(check_choice):
            seeing_records=input("ENTER THE NAME OF THE STUDENT WHOSE RECORD HAS TO BE SEEN : ")
        ...
        average_student=input("ENTER THE NAME OF THE STUDENT WHOSE AVERGAE MARKS NEED TO BE RETRIVED:",)
